...
namespace HomeStay.ManagerForms
{
    public partial class Check_in_outForm : Form
    {
        ...
        private CheckInOut cio = new CheckInOut();
        ...
        private void Check_in_outForm_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                this.ParentForm.Width = 1067;
                this.ParentForm.Height = 580;
                ...
                DataTable tableSh = this.schedule.getTimeSchedule(tmp);
                ...
                this.setting_DGV.DataSource = this.schedule.getShiftInfoBySetId(DateTime.Now);
        ...
        private void checkIn_BT_Click(object sender, EventArgs e)
        {
            ...
            if (shiftNameIdx.Item1 != -1)
            {
                ...
                if (cio.isExist(Globals.GlobalUserId, dateAndTime.Date, shiftName))
                {
                ...
                {
                    cio.insertCheckInData(Globals.GlobalUserId, dateAndTime.Date, shiftName, time_start, time_end, time_now);
                    if (temp == -1)
                    {
                        MessageBox.Show("You're late. Hurry Up", "Check In", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    }
        ...
        private void checkOut_BT_Click(object sender, EventArgs e)
        {
            var dateAndTime = DateTime.Now;
            var time_now = TimeSpan.Parse((dateAndTime.ToString("HH:mm:ss")));
            DataTable dt = cio.getDataCheckInCheckOutWith(Globals.GlobalUserId, dateAndTime.Date);
            if (dt.Rows.Count >= 1)
            {
            ...
            else
            {
                ...
                dt = cio.getDataCheckInCheckOutWith(Globals.GlobalUserId, dateAndTime.Date);
                if (dt.Rows.Count >= 1)
                {
                    foreach (DataRow row in dt.Rows)
                    {
